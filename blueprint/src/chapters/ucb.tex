...
\begin{align*}
  P\left(0 < N_{n, a} \ \wedge \ \hat{\mu}_{n, a} + \sqrt{\frac{2 c \log(n + 1)}{N_{n,a}}} \le \mu_a\right)
  &\le \frac{1}{(n + 1)^{c - 1}}
  \: .
\end{align*}
And also,
\begin{align*}
  P\left(0 < N_{n, a} \ \wedge \ \hat{\mu}_{n, a} - \sqrt{\frac{2 c \log(n + 1)}{N_{n,a}}} \ge \mu_a\right)
  &\le \frac{1}{(n + 1)^{c - 1}}
  \: .
\end{align*}
\end{lemma}
...
\begin{align*}
  ...
  \\&\quad +
      \sum_{s=1}^{n-1} \mathbb{I}\{A_s = a \ \wedge \ C < N_{s,a} \ \wedge \
        \mu^* \le \hat{\mu}_{s, a^*} + \sqrt{\frac{2 c \log(s + 1)}{N_{s,a^*}}} \ \wedge \
        \hat{\mu}_{s, A_s} - \sqrt{\frac{2 c \log(s + 1)}{N_{s,A_s}}} \le \mu_{A_s}\}
  \\&\quad +
      \sum_{s=1}^{n-1}
        \mathbb{I}\{0 < N_{s, a^*} \ \wedge \ \hat{\mu}_{s, a^*} + \sqrt{\frac{2 c \log(s + 1)}{N_{s,a^*}}} <
          \mu^*\}
  \\&\quad +
      \sum_{s=1}^{n-1}
        \mathbb{I}\{0 < N_{s, a} \ \wedge \ \mu_a <
          \hat{\mu}_{s, a} - \sqrt{\frac{2 c \log(s + 1)}{N_{s,a}}}\}
\end{align*}
\end{lemma}
...
For the UCB algorithm with parameter $c \ge 0$, for any time $n \in \mathbb{N}$ and any arm $a \in [K]$ with positive gap, the first sum in Lemma~\ref{lem:pullCount_le_add_three} is equal to zero for positive $C$ such that $C \ge \frac{8 c \log(n + 1)}{\Delta_a^2}$.
\end{lemma}
...
\begin{align*}
  \mathbb{E}[N_{n,a}]
  &\le \frac{8 c \log(n + 1)}{\Delta_a^2} + 2 + 2 \sum_{s=0}^{n-1} \frac{1}{(s + 1)^{c - 1}}
  \: .
\end{align*}
\end{lemma}
...
\begin{align*}
  R_n
  &\le \sum_{a : \Delta_a > 0} \left(\frac{8 c \log(n + 1)}{\Delta_a} + 2 \Delta_a\left(1 + \sum_{s=0}^{n-1} \frac{1}{(s + 1)^{c - 1}}\right)\right)
  \: .
\end{align*}
\end{lemma}

\begin{proof}\leanok
  \uses{def:environment,def:algorithm,lem:regret_eq_sum_pullCount_mul_gap,lem:pullCount_basic,lem:expectation_pullCount_le,def:pullCount}
