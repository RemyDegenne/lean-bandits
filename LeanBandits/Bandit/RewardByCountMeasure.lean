...
@[blueprint
  "lem:condIndepFun_reward_stepsUntil_arm"
  (statement := /-- For $t > 0$, $R_t \ind \mathbb{I}\{T_{n, a} = t\} \mid A_t$. -/)
  (proof := /-- $\mathbb{I}\{T_{n, a} = t\}$ is measurable with respect to the sigma-algebra
    generated by $(H_{t-1}, A_t)$ by Lemma~\ref{lem:measurable_comap_indicator_stepsUntil_eq}.
    
    It thus suffices to show that $R_t \ind (H_{t-1}, A_t) \mid A_t$, which is implied by $R_t \ind
    H_{t-1} \mid A_t$ (Lemma~\ref{lem:CondIndepFun.prod_right}), which is
    Lemma~\ref{lem:condIndepFun_reward_hist_action}. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:reward_cond_stepsUntil"
  (statement := /-- Let $n > 0$, $t \in \mathbb{N}$ and suppose that $P(T_{n, a} = t) > 0$.
    Then $\mathcal{L}(R_t \mid T_{n, a} = t) = \nu(a)$. -/)
  (proof := /-- First, if $T_{n, a} = t$, then $A_t = a$ (Lemma~\ref{lem:stepsUntil_basic}), such
    that $\mathcal{L}(R_t \mid T_{n, a} = t) = \mathcal{L}(R_t \mid T_{n, a} = t, A_t = a)$.
    
    Then, using first the independence from Lemma~\ref{lem:condIndepFun_reward_stepsUntil_arm} and
    then the conditional distribution from Lemma~\ref{lem:condDistrib_reward_stationaryEnv}, we have
    \begin{align*}
...
@[blueprint
  "lem:condDistrib_rewardByCount_stepsUntil"
  (statement := /-- For $n > 0$ and $t \in \mathbb{N}$, $\mathcal{L}(Y_{n,a} \mid T_{n,a}) = \nu(a)$
    (in which the measure on the r.h.s. is seen as a constant kernel). -/)
  (proof := /-- It suffices to show that for all $t \in \mathbb{N} \cup \{\infty\}$ such that
    $\mathbb{P}(T_{n, a} = t) > 0$, the law of $Y_{n,a}$ conditioned on $T_{n,a} = t$ is $\nu(a)$.
    
    If $t < \infty$, then $\mathcal{L}(Y_{n, a} \mid T_{n, a} = t) = \mathcal{L}(R_t \mid T_{n, a} =
    t) = \nu(a)$ by Lemma~\ref{lem:reward_cond_stepsUntil}.
    
    If $t = \infty$, then $\mathcal{L}(Y_{n, a} \mid T_{n, a} = \infty) = \mathcal{L}(Z_{n, a} \mid
    T_{n, a} = \infty)$. By independence of $Z_{n,a}$ and $T_{n, a}$, this is just $\nu(a)$, the law
    of $Z_{n,a}$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:hasLaw_rewardByCount"
  (statement := /-- For $n > 0$ and $a \in \mathcal{A}$, $\mathcal{L}(Y_{n,a}) = \nu(a)$. -/)
  (proof := /-- The law of $Y_{n,a}$ is given by $\mathcal{L}(Y_{n, a}) = \mathcal{L}(Y_{n, a} \mid
    T_{n, a}) \circ \mathcal{L}(T_{n, a})$.
