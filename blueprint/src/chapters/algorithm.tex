...
\begin{definition}[Step and history]\label{def:history}
  \leanok
  \lean{Learning.step, Learning.hist}
...
\begin{lemma}\label{lem:adapted_history}
  ...
  \leanok
  \lean{Learning.adapted_step, Learning.adapted_hist}
...
\begin{lemma}\label{lem:law_X_zero}
  \uses{def:history, def:trajMeasure}
  \leanok
  \lean{Learning.hasLaw_step_zero}
...
\begin{definition}\label{def:actionReward}
  \uses{def:history}
  \leanok
  \lean{Learning.action, Learning.reward}
...
\begin{lemma}\label{lem:adapted_action_reward}
  ...
  \leanok
  \lean{Learning.adapted_action, Learning.adapted_reward}
...
\begin{lemma}\label{lem:condDistrib_reward_stationaryEnv}
  ...
  \leanok
  \lean{Learning.condDistrib_reward_stationaryEnv}
...
\begin{lemma}\label{lem:condIndepFun_reward_hist_action}
  \uses{def:actionReward, def:trajMeasure, def:algorithm, def:stationaryEnv}
  \leanok
  \lean{Learning.condIndepFun_reward_hist_action}
In a stationary environment, for any $t \in \mathbb{N}$, the reward $R_{t+1}$ is conditionally independent of the history $H_t$ given the action $A_{t+1}$.
\end{lemma}
