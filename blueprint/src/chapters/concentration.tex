...
Then
\begin{align*}
  \nu(a)^{\otimes \mathbb{N}} \left( \sum_{s=0}^{k-1} (\omega_{s} - \mu_a) \le - \sqrt{2 c k \log(n + 1)} \right)
  &\le \frac{1}{(n + 1)^{c}}
  \: .
\end{align*}
The same upper bound holds for the upper tail:
\begin{align*}
  \nu(a)^{\otimes \mathbb{N}} \left( \sum_{s=0}^{k-1} (\omega_{s} - \mu_a) \ge \sqrt{2 c k \log(n + 1)} \right)
  &\le \frac{1}{(n + 1)^{c}}
  \: .
\end{align*}
\end{lemma}
