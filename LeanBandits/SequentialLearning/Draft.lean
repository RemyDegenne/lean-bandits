...
lemma adapted_pullCount_add_one' [MeasurableSingletonClass α]
    ...
    Measurable[IsAlgEnvInteraction.filtration hA hR' n] (pullCount A a (n + 1)) := by
  have : pullCount A a (n + 1) = (fun h : Iic n → α × R ↦ pullCount' n h a) ∘
      (IsAlgEnvInteraction.hist A R' n) := by
    ext
    exact pullCount_add_one_eq_pullCount'
  rw [IsAlgEnvInteraction.filtration, this]
  exact measurable_comp_comap _ (measurable_pullCount' n a)

lemma adapted_pullCount_add_one [MeasurableSingletonClass α]
    (hA : ∀ n, Measurable (A n)) (hR' : ∀ n, Measurable (R' n)) (a : α) :
    Adapted (IsAlgEnvInteraction.filtration hA hR') (fun n ↦ pullCount A a (n + 1)) :=
...
lemma isPredictable_pullCount [MeasurableSingletonClass α]
    (hA : ∀ n, Measurable (A n)) (hR' : ∀ n, Measurable (R' n)) (a : α) :
    IsPredictable (IsAlgEnvInteraction.filtration hA hR') (pullCount A a) := by
...
lemma isStoppingTime_stepsUntil [MeasurableSingletonClass α]
    ...
    IsStoppingTime (IsAlgEnvInteraction.filtration hA hR') (stepsUntil A a m) := by
  rw [stepsUntil_eq_leastGE _ hm]
  refine Adapted.isStoppingTime_leastGE _ fun n ↦ ?_
  suffices StronglyMeasurable[IsAlgEnvInteraction.filtration hA hR' n] (pullCount A a (n + 1)) by
    fun_prop
...
lemma measurable_comap_indicator_stepsUntil_eq [MeasurableSingletonClass α] [Nonempty R]
    ...
    Measurable[MeasurableSpace.comap
        (fun ω : Ω ↦ (IsAlgEnvInteraction.hist A R' (n-1) ω, A n ω)) inferInstance]
...
lemma measurableSet_stepsUntil_eq [Nonempty R] [MeasurableSingletonClass α]
    (hA : ∀ n, Measurable (A n)) (hR' : ∀ n, Measurable (R' n)) (a : α) (m n : ℕ) :
    MeasurableSet[MeasurableSpace.comap (fun ω : Ω ↦ (IsAlgEnvInteraction.hist A R' (n-1) ω, A n ω))
        inferInstance]
...
theorem isStoppingTime_stepsUntil_filtrationAction [Nonempty R] [MeasurableSingletonClass α]
    ...
    IsStoppingTime (IsAlgEnvInteraction.filtrationAction hA hR') (stepsUntil A a m) := by
  refine isStoppingTime_of_measurableSet_eq fun n ↦ ?_
  by_cases hn : n = 0
  · simp only [hn, IsAlgEnvInteraction.filtrationAction_zero_eq_comap, WithTop.coe_zero]
    exact measurableSet_stepsUntil_eq_zero a m
  · rw [IsAlgEnvInteraction.filtrationAction_eq_comap _ hn]
