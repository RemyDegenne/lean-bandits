...
\begin{proof}\leanok
  \uses{def:environment,def:detAlgorithm,def:algorithm,def:history,lem:pullCount_basic,def:etcAlgorithm}
...
Then for the Explore-Then-Commit algorithm with parameter $m$, for any arm $a \in [K]$ with $\Delta_a > 0$, we have $\mathbb{P}(\hat{A}_m^* = a) \le \exp\left(- \frac{m \Delta_a^2}{4 \sigma^2}\right)$.
\end{lemma}
...
\begin{proof}\leanok
  \uses{lem:pullCount_etcAlgorithm,def:environment,def:algorithm,lem:regret_eq_sum_pullCount_mul_gap,lem:prob_etc_error_le_exp,lem:pullCount_basic,def:pullCount}
...
\begin{align*}
  ...
  \: .
\end{align*}
It thus suffices to prove the inequality $\mathbb{P}(\hat{A}_m^* = a) \le \exp\left(- \frac{m \Delta_a^2}{4 \sigma^2}\right)$ for $\Delta_a > 0$.
