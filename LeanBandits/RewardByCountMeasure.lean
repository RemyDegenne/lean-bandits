...
@[fun_prop]
lemma measurable_pullCount (a : α) (t : ℕ) : Measurable (fun h ↦ pullCount h a t) := by
...
@[fun_prop]
lemma measurable_stepsUntil (a : α) (m : ℕ) : Measurable (fun h ↦ stepsUntil h a m) := by
  classical
  have h_union : {h' | ∃ s, pullCount h' a (s + 1) = m}
      = ⋃ s : ℕ, {h' | pullCount h' a (s + 1) = m} := by ext; simp
  have h_meas_set : MeasurableSet {h' | ∃ s, pullCount h' a (s + 1) = m} := by
    rw [h_union]
  ...
  refine Measurable.dite (s := {k' : ℕ → α × ℝ | ∃ s, pullCount k' a (s + 1) = m})
    (f := fun x ↦ (Nat.find x.2 : ℕ∞)) (g := fun _ ↦ ⊤) ?_ (by fun_prop) h_meas_set
  refine Measurable.coe_nat_enat ?_
  refine measurable_find _ fun k ↦ ?_
  suffices MeasurableSet {x : ℕ → α × ℝ | pullCount x a (k + 1) = m} by
    have : Subtype.val ''
          {x : {k' : ℕ → α × ℝ | ∃ s, pullCount k' a (s + 1) = m} | pullCount x a (k + 1) = m}
        = {x : ℕ → α × ℝ | pullCount x a (k + 1) = m} := by
      ext x
...
lemma measurable_stepsUntil' (a : α) (m : ℕ) :
    Measurable (fun ω : (ℕ → α × ℝ) × (ℕ → α → ℝ) ↦ stepsUntil ω.1 a m) :=
...
lemma measurable_rewardByCount (a : α) (m : ℕ) :
  ...
  · change Measurable ((fun p : ℕ × (ℕ → α × ℝ) ↦ reward p.1 p.2)
      ∘ (fun ω : (ℕ → α × ℝ) × (ℕ → α → ℝ) ↦ ((stepsUntil ω.1 a m).toNat, ω.1)))
    have : Measurable fun ω : (ℕ → α × ℝ) × (ℕ → α → ℝ) ↦ ((stepsUntil ω.1 a m).toNat, ω.1) :=
...
lemma condIndepFun_reward_stepsUntil_arm [StandardBorelSpace α] [Countable α] [Nonempty α]
  ...
  suffices ∃ φ : ((Iic (n - 1) → α × ℝ) × α) → ℕ, Measurable φ ∧
      ({ω : ℕ → α × ℝ | stepsUntil ω a m = ↑n}.indicator (fun _ ↦ 1))
...
lemma reward_cond_stepsUntil [StandardBorelSpace α] [Countable α] [Nonempty α] (a : α) (m n : ℕ)
    (hm : m ≠ 0)
    (hμn : (Bandit.measure alg ν) ((fun ω ↦ stepsUntil ω.1 a m) ⁻¹' {↑n}) ≠ 0) :
    𝓛[fun ω ↦ reward n ω.1 | fun ω ↦ stepsUntil ω.1 a m ← (n : ℕ∞);
      Bandit.measure alg ν] = ν a := by
  let μ := Bandit.measure alg ν
  have hμna :
      μ ((fun ω ↦ stepsUntil ω.1 a m) ⁻¹' {↑n} ∩ (fun ω ↦ arm n ω.1) ⁻¹' {a}) ≠ 0 := by
    suffices ((fun ω : (ℕ → α × ℝ) × (ℕ → α → ℝ) ↦
          stepsUntil ω.1 a m) ⁻¹' {↑n} ∩ (fun ω ↦ arm n ω.1) ⁻¹' {a})
        = (fun ω ↦ stepsUntil ω.1 a m) ⁻¹' {↑n} by simpa [this] using hμn
    ext ω
  ...
  have hμa : μ.map (fun ω ↦ arm n ω.1) {a} ≠ 0 := by
    ...
    exact arm_eq_of_stepsUntil_eq_coe hm
  calc 𝓛[fun ω ↦ reward n ω.1 | fun ω ↦ stepsUntil ω.1 a m ← (n : ℕ∞); μ]
  _ = (μ[|(fun ω ↦ stepsUntil ω.1 a m) ⁻¹' {↑n} ∩ (fun ω ↦ arm n ω.1) ⁻¹' {a}]).map
      (fun ω ↦ reward n ω.1) := by
    congr with ω
    simp only [Set.mem_preimage, Set.mem_singleton_iff, Set.mem_inter_iff, iff_self_and]
    exact arm_eq_of_stepsUntil_eq_coe hm
  _ = (μ[|{ω : (ℕ → α × ℝ) × (ℕ → α → ℝ) | stepsUntil ω.1 a m = ↑n}.indicator 1 ⁻¹' {1}
...
lemma condDistrib_rewardByCount_stepsUntil [Countable α] [StandardBorelSpace α] [Nonempty α]
    (a : α) (m : ℕ) (hm : m ≠ 0) :
    condDistrib (fun ω ↦ rewardByCount a m ω.1 ω.2) (fun ω ↦ stepsUntil ω.1 a m)
        (Bandit.measure alg ν)
      =ᵐ[(Bandit.measure alg ν).map (fun ω ↦ stepsUntil ω.1 a m)] Kernel.const _ (ν a) := by
  let μ := Bandit.measure alg ν
  refine (condDistrib_ae_eq_cond (μ := μ)
    (X := fun ω ↦ stepsUntil ω.1 a m) (by fun_prop) (by fun_prop)).trans ?_
  ...
  | top =>
    ...
    · exact indepFun_prod (X := fun ω : ℕ → α × ℝ ↦ stepsUntil ω a m)
...
lemma hasLaw_rewardByCount [Countable α] [StandardBorelSpace α] [Nonempty α]
  ...
  map_eq := by
    have h_condDistrib :
        condDistrib (fun ω ↦ rewardByCount a m ω.1 ω.2) (fun ω ↦ stepsUntil ω.1 a m)
          (Bandit.measure alg ν)
        =ᵐ[(Bandit.measure alg ν).map (fun ω ↦ stepsUntil ω.1 a m)]
    ...
    _ = (condDistrib (fun ω ↦ rewardByCount a m ω.1 ω.2) (fun ω ↦ stepsUntil ω.1 a m)
          (Bandit.measure alg ν))
        ∘ₘ ((Bandit.measure alg ν).map (fun ω ↦ stepsUntil ω.1 a m)) := by
      rw [condDistrib_comp_map (by fun_prop) (by fun_prop)]
    _ = (Kernel.const _ (ν a))
        ∘ₘ ((Bandit.measure alg ν).map (fun ω ↦ stepsUntil ω.1 a m)) :=
      Measure.comp_congr h_condDistrib
    _ = ν a := by
      have : IsProbabilityMeasure
          ((Bandit.measure alg ν).map (fun ω ↦ stepsUntil ω.1 a m)) :=
