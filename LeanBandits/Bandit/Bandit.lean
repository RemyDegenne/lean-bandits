...
@[blueprint
  "def:Bandit.measure"
  (title := "Bandit probability space")
  (statement := /-- As in Definition~\ref{def:trajMeasure}, an algorithm $(\pi, P_0)$ and bandit
    $\nu$ together defines a probability distribution $\mathbb{P}_{\mathcal{T}}$ on the space
    $\Omega_{\mathcal{T}} := (\mathcal{A} \times \mathbb{R})^{\mathbb{N}}$, the space of infinite
    sequences of arms and rewards.
    We augment that probability space with a stream of rewards from each arm, independent of the
    bandit interaction, to get the probability space $(\Omega, \mathbb{P})$, where $\Omega =
    \Omega_{\mathcal{T}} \times \mathbb{R}^{\mathbb{N} \times \mathcal{A}}$ and $\mathbb{P} =
    \mathbb{P}_{\mathcal{T}} \otimes (\bigotimes_{n \in \mathbb{N}, a \in \mathcal{A}} \nu(a))$. -/)]
noncomputable
...
@[blueprint
  "def:arrayMeasure"
  (statement := /-- Let $I = [0,1]$ and let $P_U$ be the uniform distribution on $I$. We define the
    probability space $(\Omega_{\mathcal{A}}, P_{\mathcal{A}})$, where
    \begin{align*}
      \Omega_{\mathcal{A}} &:= I^{\mathbb{N}} \times \mathcal{R}^{\mathbb{N} \times \mathcal{A}}
      \: , \\
      P_{\mathcal{A}} &:= \left( \bigotimes_{n \in \mathbb{N}} P_U \right) \otimes \left(
...
@[blueprint
  "def:algFunction"
  (statement := /-- Let $\mathfrak{A}$ be an algorithm with action space $\mathcal{A}$ and reward
    space $\mathcal{R}$, policy $\pi$ and initial distribution $P_0$.
    For $\mathcal{A}$ and $\mathcal{R}$ standard Borel spaces, there exists jointly measurable
    functions $f'_0 : I \to \mathcal{A}$ and $f_t : (\mathcal{A} \times \mathcal{R})^{t+1} \times I
    \to \mathcal{A}$ such that
    \begin{itemize}
      \item the law of $f'_0$ is $P_0$,
      \item for all history $h_t \in (\mathcal{A} \times \mathcal{R})^{t+1}$, the law of $f_t(h_t,
      \cdot)$ is $\pi_t(h_t)$.
    \end{itemize} -/)]
noncomputable
...
@[blueprint
  "def:AM.history"
  (statement := /-- The history, actions and rewards on the array model probability space
    $(\Omega_{\mathcal{A}}, P_{\mathcal{A}})$ are defined as follows:
    \begin{itemize}
      \item the action at time $0$ is $A_0(\omega) = f'_0(\omega_{1,0})$, the reward at time $0$ is
      $R_0(\omega) = \omega_{2,0,A_0(\omega)}$, and the history at time $0$ is $H_0(\omega) =
      (A_0(\omega), R_0(\omega))$,
      \item for $t \ge 0$, the action at time $t+1$ is $A_{t+1}(\omega) = f_t(H_t(\omega),
      \omega_{1,t+1})$, the reward at time $t+1$ is $R_{t+1}(\omega) =
      \omega_{2,N_{t+1,A_{t+1}(\omega)},A_{t+1}(\omega)}$, and the history at time $t+1$ is
      $H_{t+1}(\omega) = (H_t(\omega), (A_{t+1}(\omega), R_{t+1}(\omega)))$.
    \end{itemize} -/)]
noncomputable
...
@[fun_prop, blueprint
  "lem:AM.measurable_hist"
  (title := "Measurability")
  (statement := /-- $H_t$, $N_{t,A_t}$, $A_t$ and $R_t$ are measurable for all $t \in \mathbb{N}$.
    -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.hist_congr"
  (title := "Congruence for the history")
  (statement := /-- Let $\omega, \omega' \in \Omega_{\mathcal{A}}$ and $t \in \mathbb{N}$.
    Suppose that
    \begin{align*}
...
@[blueprint
  "lem:AM.stepsUntil_congr"
  (title := "Congruence for the number of pulls and action")
  (statement := /-- Let $\omega, \omega' \in \Omega_{\mathcal{A}}$, $t, m \in \mathbb{N}$ and $a \in
    \mathcal{A}$.
    Suppose that
    \begin{align*}
      \omega_{1} &= \omega'_{1}
      ...
      \: .
    \end{align*}
    Then $(N_{t+1, a}(\omega) = m \wedge A_{t+1}(\omega) = a) \iff (N_{t+1, a}(\omega') = m \wedge
    A_{t+1}(\omega') = a)$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.measurable_hist_todo"
  (statement := /-- For all $t \in \mathbb{N}$, $H_t$ is measurable with respect to the
    sigma-algebra generated by $F_{1, t}$, and with respect to the sigma-algebra generated by $F_{2,
    a, t}$ for any arm $a \in \mathcal{A}$. -/)
  (latexEnv := "lemma")]
...
time `n`. -/
@[blueprint
  "def:AM.probSpaceSubsets"
  (statement := /-- We define the following functions on $\Omega_{\mathcal{A}}$:
    \begin{align*}
      F_{1, t}(\omega) &= ((\omega_{1,s})_{s \le t}, (\omega_{2,s})_{s \in \mathbb{N}})
      \: , \\
      F_{2, a, t}(\omega) &= ((\omega_{1,s})_{s \in \mathbb{N}}, (\omega_{2,
      \min\{s,N_{t+1,a}(\omega)-1\}, a})_{s \in \mathbb{N}}, (\omega_{2, s, b})_{s \in \mathbb{N}, b
      \ne a})
      \: , \\
      F_{2, a}^m(\omega) &= ((\omega_{1,s})_{s \in \mathbb{N}}, (\omega_{2, \min\{s,m-1\}, a})_{s
      \in \mathbb{N}}, (\omega_{2, s, b})_{s \in \mathbb{N}, b \ne a})
      \: .
    \end{align*}
    In the definition of $F_{2, a, t}$ and $F_{2, a}^m$, if $N_{t+1,a}(\omega) = 0$ (resp. $m = 0$),
    then the second component is a constant sequence equal to an arbitrary value.
    
    $F_{1, t}(\omega)$ contains all the information in $\omega$ except for the action selection
    randomness after time $t$.
    
    $F_{2, a, t}(\omega)$ contains all the information in $\omega$ except the rewards for arm $a$
    indexed by $N_{t+1,a}(\omega)$ or more.
...
@[blueprint
  "lem:AM.measurable_action_add_one_truePast"
  (statement := /-- $A_{t+1}$ is measurable with respect to the sigma-algebra generated by $F_{2, a,
    t}$ for any arm $a \in \mathcal{A}$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.measurable_pullCount_add_one_truePast"
  (statement := /-- $N_{t+1,a}$ is measurable with respect to the sigma-algebra generated by $F_{2,
    a, t}$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.measurable_stepsUntil"
  (statement := /-- For $t, m \in \mathbb{N}$ and $a \in \mathcal{A}$, the indicator function
    $\mathbb{I}\{N_{t+1,a} = m \wedge A_{t+1} = a\} : \Omega_{\mathcal{A}} \to \{0, 1\}$ is
    measurable with respect to the sigma-algebra generated by $F_{2, a}^m$. -/)
  (latexEnv := "lemma")]
...
omit [Nonempty R] in
@[blueprint
  "lem:AM.measurable_pullCount_action_add_one_hist"
  (statement := /-- For $t \in \mathbb{N}$, the function $N_{t+1, A_{t+1}}$ is measurable with
    respect to the sigma-algebra generated by $H_t$ and $A_{t+1}$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.indepFun_fst_add_one_aux"
  (statement := /-- $\omega \mapsto \omega_{1, t+1}$ is independent of $F_{1, t}$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.indepFun_fst_add_one_hist"
  (statement := /-- $\omega \mapsto \omega_{1, t+1}$ is independent of $H_t$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.indepFun_snd_apply_aux"
  (statement := /-- For $a \in \mathcal{A}$ and $m \in \mathbb{N}$, $\omega \mapsto \omega_{2, m,
    a}$ is independent of $F_{2, a}^m$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (statement := /-- For $a \in \mathcal{A}$, $\omega \mapsto \omega_{2, m, a}$ is independent of the
    indicator function $\mathbb{I}\{N_{t+1,a} = m \wedge A_{t+1} = a\}$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (statement := /-- For $a \in \mathcal{A}$ and $t, m \in \mathbb{N}$, $\omega \mapsto \omega_{2, m,
    a}$ is independent of $H_t$ given that $N_{t+1,a} = m$ and $A_{t+1} = a$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.hasLaw_action_zero"
  (statement := /-- The law of $A_0$ in the array model is $P_0$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.hasCondDistrib_reward_zero"
  (statement := /-- In the array model, $P_{\mathcal{A}}[R_0 \mid A_0] = \nu$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (statement := /-- In the array model, $P_{\mathcal{A}}[R_{t+1} \mid N_{t+1,A_{t+1}}, A_{t+1}] =
    \nu$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.hasCondDistrib_reward_hist_action_pullCount"
  (statement := /-- In the array model, $P_{\mathcal{A}}[R_{t+1} \mid H_t, A_{t+1}, N_{t+1,A_{t+1}}]
    = \nu$. -/)
  (latexEnv := "lemma")]
...
time `n + 1`. -/
@[blueprint
  "lem:AM.condIndepFun_reward_hist"
  (statement := /-- For $t \ge 0$, $R_{t+1} \ind H_t \mid A_{t+1}, N_{t+1, A_{t+1}}$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "lem:AM.hasCondDistrib_action"
  (statement := /-- In the array model, $P_{\mathcal{A}}[A_{t+1} \mid H_t] = \pi_t$. -/)
  (latexEnv := "lemma")]
lemma hasCondDistrib_action (alg : Algorithm α R) (ν : Kernel α R) [IsMarkovKernel ν] (n : ℕ) :
...
@[blueprint
  "lem:AM.hasCondDistrib_reward"
  (statement := /-- In the array model, $P_{\mathcal{A}}[R_{t+1} \mid H_t, A_{t+1}] = \nu$. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (statement := /-- The actions and rewards defined on the array model probability space
    $(\Omega_{\mathcal{A}}, P_{\mathcal{A}})$ form an algorithm-environment sequence for the
    algorithm $\mathfrak{A}$ and bandit $\nu$. -/)
  (proof := /-- The four conditions of Definition~\ref{def:IsAlgEnvSeq} are satisfied by
    Lemmas~\ref{lem:AM.hasLaw_action_zero}, \ref{lem:AM.hasCondDistrib_reward_zero},
    \ref{lem:AM.hasCondDistrib_action} and \ref{lem:AM.hasCondDistrib_reward}. -/)]
